...
import net.floodlightcontroller.currentstate.CurrentState;
...
extends IdleStateAwareChannelHandler {
	...
	enum ChannelState {
		...
		void processOFEchoReply(OFChannelHandler h, OFEchoReply m)
				throws IOException {
			//if id == 777 it means is for calculating the rrt between switches and the controller
			if(m.getXid() == 777){
				try {
					long end = System.currentTimeMillis();
					CurrentState.addControllerRTT(h.sw.getId(), end);
				}catch(Exception e){
